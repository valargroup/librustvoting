...
#[tokio::test]
async fn a_committed_vote_never_dispatched_prepares_its_plan_before_the_chain() {
    // The vote for proposal 1 was persisted but its cast step failed before
    // any POST was reserved and before a helper plan existed. Resuming it
    // must make the plan durable first, as the cast would have, so a
    // confirmation cannot land on a vote without a plan.
    let database = crate::share_tracking::tests::db_with_share(&[]);
    database
        .conn()
        .execute(
            "UPDATE votes SET tx_hash = NULL, vc_tree_position = NULL
    ...
    let events = progress.events.lock().unwrap();
    let plans_prepared = events
        .iter()
        .position(|event| matches!(event, crate::RoundStepProgress::HelperPlansPrepared(_)));
    assert!(
        plans_prepared.is_some(),
        "the plan is made durable before the broadcast: {events:?}"
    );
    assert!(
        chain.requests.load(std::sync::atomic::Ordering::SeqCst) > 0,
        "the chain is still reconciled afterwards"
    );
}
