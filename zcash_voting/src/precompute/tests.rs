...
#[test]
fn a_client_a_caller_still_holds_is_not_pruned_before_its_first_sync() {
    let db = db("wallet-in-flight");
    // The caller's only transport clone moved into the client, and no round
    // has been synced yet: nothing but the caller's handle keeps it alive.
    let in_flight = vote_tree_for(&db, Some(CountingTransport::new())).unwrap();
    assert!(in_flight.cached_rounds().is_empty());

    // A concurrent registry access must not evict it.
    assert!(cached_vote_tree_rounds(&db).is_empty());
    let found = vote_tree_for_round(&db, ROUND_ID).unwrap();
    assert!(
        Arc::ptr_eq(&in_flight, &found),
        "the in-flight client must still be the wallet's client"
    );
    drop(found);
    drop(in_flight);
    reset_vote_tree(&db, "").unwrap();
