...
//! tree or a stale route. An entry is also kept while any caller holds its
//! client, so a sync in flight cannot be evicted before it creates its round
//! state. A routed client
...
impl VoteTreeEntry {
    fn is_reachable(&self) -> bool {
        ...
        // A caller still holding the client (a sync in flight before its
        // first round client exists) must find the same entry afterwards.
        let held_by_a_caller = Arc::strong_count(&self.sync) > 1;
        connection_is_live && (transport_is_held || holds_round_state || held_by_a_caller)
    }
