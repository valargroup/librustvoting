...
/// Byte length of voting hotkey seed material returned by
/// [`VotingHotkey::secret_seed`].
pub const VOTING_HOTKEY_SEED_LEN: usize = 64;
...
/// store [`VotingHotkey::secret_seed`] in platform secure storage, and
/// reconstruct it later with [`voting_hotkey_from_seed`]. This keeps wallet
/// root seed and mnemonic material out of the voting hotkey API. The resulting
/// hotkey is not deterministic across fresh app installs unless the stored
/// hotkey seed is restored.
///
/// # Errors
///
...
pub fn generate_random_voting_hotkey(network: Network) -> Result<VotingHotkey, VotingError> {
    let mut seed = Zeroizing::new(vec![0u8; VOTING_HOTKEY_SEED_LEN]);
...
///
/// `hotkey_seed` must be seed material previously returned by
/// [`VotingHotkey::secret_seed`] or generated by an equivalent wallet-owned
/// CSPRNG. It is not the wallet root seed or mnemonic-derived seed.
///
/// # Errors
///
/// Returns [`VotingError::InvalidInput`] when `hotkey_seed` is too short or cannot
...
pub fn voting_hotkey_from_seed(
    ...
    if hotkey_seed.len() < 32 {
        return Err(VotingError::InvalidInput {
            message: format!(
                "hotkey seed must be at least 32 bytes, got {}",
                hotkey_seed.len()
            ),
        });
    }
...
mod tests {
    ...
    fn random_hotkey_returns_storable_secret_seed() {
        ...
        assert_eq!(first.secret_seed().len(), VOTING_HOTKEY_SEED_LEN);
        assert_eq!(second.secret_seed().len(), VOTING_HOTKEY_SEED_LEN);
    ...
    fn short_hotkey_seed_is_rejected() {
        ...
        assert!(err.contains("hotkey seed must be at least 32 bytes"));
    }
}
