...
pub(crate) const CURRENT_VERSION: u32 = 12;
...
mod tests {
    ...
    fn pre_v12_schema() -> String {
        include_str!("migrations/001_init.sql")
            .replace("    pczt_bytes          BLOB,\n", "")
            .replace("    pczt_action_index   INTEGER,\n", "")
            .replace("    delegation_action_bytes BLOB,\n", "")
    }
    ...
    #[test]
    fn test_migrate_from_v11_schema_recreates_pczt_setup_columns() {
        let mut conn = Connection::open_in_memory().unwrap();
        conn.execute_batch(&pre_v12_schema()).unwrap();
        queries::insert_round(&conn, "wallet", &test_params(), None).unwrap();
        queries::insert_bundle(&conn, "test-round", "wallet", 0, &[1]).unwrap();
