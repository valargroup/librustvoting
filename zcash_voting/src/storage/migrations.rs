...
/// [`LAUNCH_VERSION`] to [`CURRENT_VERSION`]. Migrations must preserve any state
/// that can indicate a completed or ambiguous external effect. Intentional
/// cleanup must be limited to locally rebuildable state. `001_init.sql` is
/// updated alongside so a fresh database and a migrated one end up with the
/// same schema.
...
END;",
    ...
    (18, LEGACY_DELEGATION_RECONCILIATION_SQL),
];

/// Repair released local delegation state after durable PCZT storage exists.
/// Submitted, confirmed, and imported rows are outside this migration's scope.
...
impl LegacyKeystoneSignatureCandidate {
    fn matches_setup_and_verifies(&self) -> bool {
        self.stored_sighash.as_deref() == Some(self.sighash.as_slice())
            ...
            && super::operations::verify_delegation_spend_auth_signature(
                &self.randomized_key,
                &self.sighash,
                &self.signature,
            )
...
mod tests {
    ...
    fn migrate_v17_adds_pczt_then_reconciles_legacy_proof() {
        ...
        assert!(
            queries::get_keystone_signatures(&conn, "test-round", "wallet")
                .unwrap()
                .is_empty()
        );
    }
    ...
    #[test]
    fn migrate_v18_repairs_legacy_setup_proofs_and_signatures() {
        let mut conn = Connection::open_in_memory().unwrap();
