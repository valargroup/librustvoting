...
use crate::{
    ...
    round_auth::RoundAuthDigestV3,
    types::{Network, RoundBoundVotingHotkeyTarget, VotingError, VotingHotkeyTarget},
};
...
const ROOT_VERIFICATION_DOMAIN: &[u8] = b"zcash_voting/authority-root-verification/v1";
...
/// Bundle material source selected independently of root custody.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum BundleMaterialSourceV1 {
    RecoverableSelfCustody,
    CustodyCapability,
}

/// Immutable public authority selection retained before a round may be used.
///
/// The authenticated-round digest commits the exact signed round payload. The
/// verification tag lets restore reject a mismatched root, source, bundle
/// source, or context without storing the root in public metadata. Encryption,
/// rollback protection, and lifecycle state remain external wallet concerns.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct VotingAuthoritySelectionV1 {
    context: VotingAuthorityContextV1,
    authority_source: VotingAuthorityRootSourceV1,
    bundle_source: BundleMaterialSourceV1,
    authenticated_round_digest: [u8; 32],
    root_verification_tag: [u8; 32],
}

impl VotingAuthoritySelectionV1 {
    /// Binds one root and both source choices to a trusted-key-verified round.
    pub fn bind(
        root: &VotingAuthorityRootV1,
        bundle_source: BundleMaterialSourceV1,
        authenticated_round: &VerifiedRoundAuthV3,
    ) -> Result<Self, VotingError> {
        if authenticated_round.round_id() != root.context.vote_round_id()
            || authenticated_round.context().network() != root.context.network()
            || authenticated_round.context().vote_chain_id() != root.context.vote_chain_id()
        {
            return Err(VotingError::InvalidInput {
                message: "authenticated round payload does not match voting authority context"
                    .to_string(),
            });
        }
        let authenticated_round_digest = authenticated_round.digest();
        let mut selection = Self {
            context: root.context.clone(),
            authority_source: root.source,
            bundle_source,
            authenticated_round_digest: *authenticated_round_digest.as_bytes(),
            root_verification_tag: [0; 32],
        };
        selection.root_verification_tag = keyed_hash32(
            root.secret_bytes(),
            ROOT_VERIFICATION_DOMAIN,
            &selection.verification_fields(),
        );
        Ok(selection)
    }
    ...
    pub fn bundle_source(&self) -> BundleMaterialSourceV1 {
        self.bundle_source
    }

    pub fn authenticated_round_digest(&self) -> RoundAuthDigestV3 {
        RoundAuthDigestV3::from_bytes(self.authenticated_round_digest)
    }

    /// Verifies that the current trusted-key-verified round is the exact round
    /// frozen into this authority selection.
    pub fn validate_current_round(
        &self,
        current_round: &VerifiedRoundAuthV3,
    ) -> Result<(), VotingError> {
        if self
            .authenticated_round_digest
            .ct_eq(current_round.digest().as_bytes())
            .into()
        {
            Ok(())
        } else {
            Err(VotingError::InvalidInput {
                message:
                    "current authenticated round payload does not match voting authority selection"
                        .to_string(),
            })
        }
    }
    ...
    /// Serializes the public selection with a strict versioned schema.
    pub fn to_json(&self) -> Result<String, VotingError> {
        serde_json::to_string(&VotingAuthoritySelectionDto::from(self)).map_err(|error| {
            VotingError::Internal {
                message: format!("failed to serialize voting authority selection: {error}"),
            }
        })
    }

    /// Parses and validates the strict version 1 public selection schema.
    pub fn from_json(json: &str) -> Result<Self, VotingError> {
        let dto: VotingAuthoritySelectionDto =
            serde_json::from_str(json).map_err(|error| VotingError::InvalidInput {
                message: format!("invalid voting authority selection JSON: {error}"),
            })?;
        dto.try_into()
    }
    ...
    fn verification_fields(&self) -> Vec<Vec<u8>> {
        ...
        fields.push(vec![match self.bundle_source {
            BundleMaterialSourceV1::RecoverableSelfCustody => 0,
            BundleMaterialSourceV1::CustodyCapability => 1,
        }]);
        fields.push(self.authenticated_round_digest.to_vec());
        fields
    }
}
...
struct VotingAuthoritySelectionDto {
    version: u32,
    ...
    authenticated_round_digest: String,
...
impl From<&VotingAuthoritySelectionV1> for VotingAuthoritySelectionDto {
    fn from(selection: &VotingAuthoritySelectionV1) -> Self {
        Self {
            version: 1,
            zcash_network: network_name(selection.context.network()).to_string(),
            account_index: selection.context.account_index(),
            orchard_fvk_fingerprint: hex::encode(selection.context.orchard_fvk_fingerprint()),
            vote_chain_id: selection.context.vote_chain_id().to_string(),
            vote_round_id: hex::encode(selection.context.vote_round_id()),
            authority_source: match selection.authority_source {
                VotingAuthorityRootSourceV1::SoftwareRegisteredKey { application } => {
            ...
            bundle_source: match selection.bundle_source {
                BundleMaterialSourceV1::RecoverableSelfCustody => {
                    "recoverable-self-custody".to_string()
                }
                BundleMaterialSourceV1::CustodyCapability => "custody-capability".to_string(),
            },
            authenticated_round_digest: hex::encode(selection.authenticated_round_digest),
            root_verification_tag: hex::encode(selection.root_verification_tag),
        }
    }
}

impl TryFrom<VotingAuthoritySelectionDto> for VotingAuthoritySelectionV1 {
    type Error = VotingError;

    fn try_from(dto: VotingAuthoritySelectionDto) -> Result<Self, Self::Error> {
        if dto.version != 1 {
            return Err(VotingError::InvalidInput {
                message: format!(
                    "unsupported voting authority selection version {}",
                    dto.version
                ),
            });
        }
        ...
        let bundle_source = match dto.bundle_source.as_str() {
            "recoverable-self-custody" => BundleMaterialSourceV1::RecoverableSelfCustody,
            "custody-capability" => BundleMaterialSourceV1::CustodyCapability,
            value => {
                return Err(VotingError::InvalidInput {
                    message: format!("unsupported bundle_source {value:?}"),
                });
            }
        };
        Ok(Self {
            ...
            authority_source,
            bundle_source,
            authenticated_round_digest: parse_lower_hex_32(
                "authenticated_round_digest",
                &dto.authenticated_round_digest,
            )?,
...
impl VotingAuthorityRootV1 {
    ...
    /// Restores a retained root only when it matches its immutable selection.
    pub fn restore(
        ...
        let root = Self {
            ...
            source: selection.authority_source,
        };
        ...
        if actual_tag.ct_eq(&selection.root_verification_tag).into() {
            Ok(root)
        } else {
            Err(VotingError::InvalidInput {
                message: "retained voting authority root does not match selection".to_string(),
            })
        }
    }
    ...
    /// Checks this live root against an immutable public selection.
    pub fn validate_selection(
        &self,
        selection: &VotingAuthoritySelectionV1,
    ) -> Result<(), VotingError> {
        if self.context != selection.context || self.source != selection.authority_source {
            return Err(VotingError::InvalidInput {
                message: "voting authority root context or source does not match selection"
                    .to_string(),
            });
        }
        let actual_tag = keyed_hash32(
        ...
        if actual_tag.ct_eq(&selection.root_verification_tag).into() {
            ...
            Err(VotingError::InvalidInput {
                message: "voting authority root does not match selection".to_string(),
            })
        }
    }

    /// Checks both the retained root binding and the currently authenticated
    /// round payload before this authority is used.
    pub fn validate_current_selection(
        &self,
        selection: &VotingAuthoritySelectionV1,
        current_round: &VerifiedRoundAuthV3,
    ) -> Result<(), VotingError> {
        self.validate_selection(selection)?;
        selection.validate_current_round(current_round)
    }
...
impl KeystoneMasterGenerationV1 {
    ...
    /// bytes and the selection's constant-time root verification tag.
    pub fn restore_retained_round_root(
        &self,
        selection: &VotingAuthoritySelectionV1,
        retained_root_secret: [u8; VOTING_AUTHORITY_ROOT_LEN],
    ) -> Result<VotingAuthorityRootV1, VotingError> {
        if selection.context.scope() != &self.scope
            || selection.authority_source
                != (VotingAuthorityRootSourceV1::KeystoneMasterGeneration {
                    generation_id: self.generation_id,
                })
        {
            return Err(VotingError::InvalidInput {
                message: "authority selection does not match Keystone master generation"
                    .to_string(),
            });
        }
        let derived = self.derive_round_root(&selection.context)?;
        if !bool::from(
            derived
                .secret_bytes()
        ...
        VotingAuthorityRootV1::restore(selection, retained_root_secret)
    }
}
...
mod tests {
    ...
    fn shared_json_vector_matches_all_authority_derivations() {
        ...
        let round_auth =
            crate::config::test_verified_round_auth_v3(crate::round_auth::RoundAuthPayloadV3::new(
                *context.vote_round_id(),
                decode_hex_array(&vector.round_auth.ea_pk),
                vector.round_auth.pir_layout,
        ...
        let vector_selection = VotingAuthoritySelectionV1::bind(
            &root,
            match vector.selection.bundle_source.as_str() {
                "recoverable-self-custody" => BundleMaterialSourceV1::RecoverableSelfCustody,
                other => panic!("unsupported test vector bundle source {other}"),
            },
            &round_auth,
        )
        .unwrap();
        assert_eq!(
            vector_selection.authenticated_round_digest().to_hex(),
            vector.selection.authenticated_round_digest
        );
        assert_eq!(
            hex::encode(vector_selection.root_verification_tag()),
            vector.selection.root_verification_tag
        );
        assert_eq!(
            vector_selection.to_json().unwrap(),
            vector.selection.canonical_json
        );
        assert_eq!(
            VotingAuthoritySelectionV1::from_json(&vector.selection.canonical_json).unwrap(),
            vector_selection
        );

        let notes = (0..vector.bundle.note_count)
            .map(|position| crate::types::NoteInfo {
                commitment: vec![position as u8 + 1; 32],
                nullifier: vec![position as u8 + 17; 32],
                value: vector.bundle.note_value,
                position,
        ...
        let bundle = plan.bundle(vector.bundle.bundle_index).unwrap();
        assert_eq!(
            hex::encode(bundle.identity().canonical_transcript()),
            vector.bundle.identity_transcript
        );
        assert_eq!(
            hex::encode(bundle.derive_van_blinding(&root).as_bytes()),
            vector.bundle.van_blinding
        );
        assert_eq!(
            hex::encode(bundle.derive_van_blinding(&software_root).as_bytes()),
            vector.registered_request.bundle_van_blinding
        );
    }
    ...
    #[test]
    fn selection_json_is_strict_and_restores_matching_root() {
        let context = context();
        let master = KeystoneMasterGenerationV1::from_new_secret(
            context.scope().clone(),
            [0x33; KEYSTONE_MASTER_SECRET_LEN],
        ...
        let decoded = VotingAuthoritySelectionV1::from_json(&json).unwrap();
        assert_eq!(decoded, selection);
        root.validate_selection(&decoded).unwrap();
        decoded.validate_current_round(&round_auth).unwrap();
        let mut stale_round_id = [0u8; 32];
        stale_round_id[0] = 3;
        let stale_context = VotingAuthorityContextV1::from_fingerprint(
            root.context().network(),
            root.context().account_index(),
            *root.context().orchard_fvk_fingerprint(),
            root.context().vote_chain_id(),
            stale_round_id,
        )
        .unwrap();
        let stale_round = crate::recoverable_authority::test_verified_round_auth_v3(&stale_context);
        assert!(decoded.validate_current_round(&stale_round).is_err());
        ...
        assert!(VotingAuthoritySelectionV1::from_json(&value.to_string()).is_err());

        let uppercase = json.replace(
            &hex::encode(master.generation_id()),
            &hex::encode_upper(master.generation_id()),
        );
        assert!(VotingAuthoritySelectionV1::from_json(&uppercase).is_err());
    }
}
