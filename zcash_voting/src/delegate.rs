...
pub struct PrepareRecoverableDelegationBundleV1Params<'a> {
    ...
    pub authority_selection: crate::recoverable_authority::VotingAuthoritySelectionV1,
    /// Exact round and snapshot produced by trusted-key v3 verification.
    pub verified_round: &'a crate::config::VerifiedVotingRoundV3,
    pub bundle_index: u32,
}
...
fn validate_recoverable_snapshot_binding<C, P, CL, R>(
    ...
    verified_round: &crate::config::VerifiedVotingRoundV3,
) -> Result<(), VotingError>
where
    C: Borrow<rusqlite::Connection>,
    P: Parameters,
{
    if &lwd.round_params != verified_round.round_params() {
        return Err(VotingError::InvalidInput {
            message: "verified round parameters do not match delegation round".to_string(),
        });
    }
    let authenticated_round = verified_round.round_auth();
    let authenticated_context = authenticated_round.context();
    if authenticated_context.network() != lwd.network
        || lwd.round_params.vote_round_id != hex::encode(authenticated_round.round_id())
    {
        return Err(VotingError::InvalidInput {
            message: "authenticated round payload does not match delegation round".to_string(),
        });
    }
    if authenticated_context.snapshot_height() != lwd.round_params.snapshot_height {
        return Err(VotingError::InvalidInput {
            message: format!(
                "authenticated snapshot height {} does not match delegation snapshot height {}",
                authenticated_context.snapshot_height(),
    ...
    if tree_state.height != authenticated_context.snapshot_height() {
        return Err(VotingError::InvalidInput {
            message: format!(
                "TreeState height {} does not match authenticated snapshot height {}",
                tree_state.height,
                authenticated_context.snapshot_height()
            ),
        });
    ...
    let authenticated_hash = hex::encode(authenticated_context.snapshot_block_hash());
    if tree_state.hash != authenticated_hash {
        return Err(VotingError::InvalidInput {
            message: "TreeState hash does not match authenticated snapshot block hash".to_string(),
        });
    }

    let snapshot_height = u32::try_from(authenticated_context.snapshot_height())
        .map(BlockHeight::from_u32)
        .map_err(|_| VotingError::InvalidInput {
            message: format!(
                "authenticated snapshot height {} does not fit in u32",
                authenticated_context.snapshot_height()
            ),
        })?;
    let fully_scanned_height = wallet_fully_scanned_height(wallet_db)?;
    if fully_scanned_height < authenticated_context.snapshot_height() {
        return Err(VotingError::InvalidInput {
            message: format!(
                "wallet is not fully scanned to authenticated snapshot height {}. Fully scanned height is {}.",
                authenticated_context.snapshot_height(),
    ...
    let wallet_hash = wallet_db
        ...
        .ok_or_else(|| VotingError::InvalidInput {
            message: format!(
                "wallet has no retained block hash at authenticated snapshot height {}",
                authenticated_context.snapshot_height()
            ),
        })?;
    if wallet_hash.to_string() != authenticated_hash {
        return Err(VotingError::InvalidInput {
            message: "wallet snapshot block hash does not match authenticated snapshot block hash"
                .to_string(),
        });
    }
    Ok(())
...
///
/// The immutable authority selection and canonical bundle material are retained
/// together. `setup` is therefore deterministic by construction.
pub struct PreparedRecoverableDelegationBundleV1<'a> {
    ...
    authority_selection: crate::recoverable_authority::VotingAuthoritySelectionV1,
...
/// longer retains the authenticated snapshot block hash needed to prove that
/// its scan and the supplied lightwalletd tree state are on the signed fork.
...
where
    ...
    P: Parameters,
{
    params.authority_root.validate_current_selection(
        &params.authority_selection,
        params.verified_round.round_auth(),
    )?;
    if params.authority_selection.bundle_source()
        != crate::recoverable_authority::BundleMaterialSourceV1::RecoverableSelfCustody
    {
        return Err(VotingError::InvalidInput {
            message: "authority selection does not permit recoverable self-custody bundles"
                .to_string(),
        });
    }
    ...
    validate_recoverable_snapshot_binding(wallet_db, &params.lwd, params.verified_round)?;
    ...
    Ok(PreparedRecoverableDelegationBundleV1 {
        ...
        authority_selection: params.authority_selection,
        recoverable_bundle,
        verified_round: params.verified_round,
    })
}
...
impl PreparedRecoverableDelegationBundleV1<'_> {
    fn bundle_use(&self) -> crate::recoverable_authority::RecoverableBundleUseV1<'_> {
        crate::recoverable_authority::RecoverableBundleUseV1::new(
            self.authority_root,
            &self.authority_selection,
            self.verified_round.round_auth(),
    ...
    where
        ...
        P: Parameters,
    {
        pir_client.validate_round(self.verified_round)?;
        self.prepared
...
mod tests {
    ...
    fn recoverable_test_material<C, P, CL, R>(
        ...
        crate::recoverable_authority::VotingAuthoritySelectionV1,
    )
    where
        C: Borrow<rusqlite::Connection>,
        P: Parameters,
    {
        ...
        let authenticated_round = recoverable_test_round_auth(round_params, snapshot_block_hash);
        let selection = crate::recoverable_authority::VotingAuthoritySelectionV1::bind(
            &root,
            crate::recoverable_authority::BundleMaterialSourceV1::RecoverableSelfCustody,
            authenticated_round.round_auth(),
        )
        .unwrap();
        (root, authenticated_round, selection)
    }

    fn recoverable_test_round_auth(
        round_params: &crate::VotingRoundParams,
        snapshot_block_hash: [u8; 32],
    ) -> crate::config::VerifiedVotingRoundV3 {
        let round_auth =
            crate::config::test_verified_round_auth_v3(crate::round_auth::RoundAuthPayloadV3::new(
                hex::decode(&round_params.vote_round_id)
                    ...
                    .try_into()
                    .unwrap(),
                round_params.ea_pk.as_slice().try_into().unwrap(),
        ...
        crate::config::test_verified_voting_round_v3(&round_auth, round_params.clone())
    }
    ...
    fn recoverable_prepare_rejects_tree_state_hash_mismatch_before_db_writes() {
        ...
        let (root, authenticated_round, selection) =
        ...
        let error = prepare_recoverable_delegation_bundle_v1(
            ...
            PrepareRecoverableDelegationBundleV1Params {
                ...
                authority_root: &root,
                authority_selection: selection,
                verified_round: &authenticated_round,
                bundle_index: 0,
            },
        )
        .err()
    ...
    fn recoverable_snapshot_binding_normalizes_wallet_hash_byte_order() {
        ...
        let authenticated_round = recoverable_test_round_auth(&round_params, expected_display_hash);
        let lwd = recoverable_test_lwd_inputs(round_params, expected_display_hash);

        validate_recoverable_snapshot_binding(&wallet_db, &lwd, &authenticated_round).unwrap();
    }
    ...
    fn recoverable_snapshot_binding_rejects_missing_retained_wallet_hash() {
        ...
        let authenticated_round = recoverable_test_round_auth(&round_params, [0xAB; 32]);
        let lwd = recoverable_test_lwd_inputs(round_params, [0xAB; 32]);

        let error = validate_recoverable_snapshot_binding(&wallet_db, &lwd, &authenticated_round)
    ...
    fn recoverable_delegation_accepts_only_deterministic_persisted_setup() {
        ...
        let verified_round = recoverable_test_round_auth(&round_params, [0xAB; 32]);
        let selection = crate::recoverable_authority::VotingAuthoritySelectionV1::bind(
            &root,
            crate::recoverable_authority::BundleMaterialSourceV1::RecoverableSelfCustody,
            verified_round.round_auth(),
