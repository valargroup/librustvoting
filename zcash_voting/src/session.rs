...
/// Opt-in behavior for [`resume_plan_with_options`].
///
/// Defaults preserve the existing recovery contract, where a share accepted by
/// at least one helper no longer blocks the foreground vote flow.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(default)]
pub struct ResumePlanOptions {
    /// Keep the submission-wide largest active share blocking until confirmed.
    ///
    /// Its outstanding share step is returned first. Other helper-share
    /// submission and confirmation steps remain nonblocking background work.
    ///
    /// Pair this with immediate largest-share scheduling. Without that
    /// scheduling opt-in, the selected share can retain a future `submit_at`
    /// time and therefore keep the caller waiting until then.
    pub require_largest_share_confirmation: bool,
}
...
pub fn resume_plan(
    ...
    resume_plan_with_options(db, round_id, proposal_ids, ResumePlanOptions::default())
}
...
/// its `SubmitShares` or `ConfirmShare` step is outstanding, and that step is
/// returned first. The planner uses the same value and identity ordering as
/// `share::plan_vote_share_submissions`. It does not select the share until
/// every proposal has a terminal ballot intent, every chosen vote is confirmed,
/// and all recovery bundles are available. Missing, unaccepted, or unconfirmed
/// non-largest shares remain in the plan as nonblocking background recovery
/// work. The wallet executes the first step, persists confirmation after any one
/// recorded helper reports it in committed chain state, and calls this function
/// again. Callers own their wait timeout; timing out must leave the durable
/// share row intact so background polling can continue.
pub fn resume_plan_with_options(
    ...
    let required_largest_share_key = if options.require_largest_share_confirmation
        && open_proposals.is_empty()
        && all_choice_votes_confirmed
    {
        ...
        if options.require_largest_share_confirmation {
            ...
            match step {
                NextStep::ConfirmShare {
                    ...
                    proposal_id,
                    share_index,
                } => blocking_confirm_share_keys.contains(&(
                    *bundle_index,
                    *proposal_id,
                    *share_index,
                )),
...
fn share_step_key(step: &NextStep) -> Option<(u32, u32, u32)> {
    match step {
        NextStep::SubmitShares {
            bundle_index,
            proposal_id,
...
mod tests {
    ...
    fn require_largest_share_confirmation() -> ResumePlanOptions {
        ResumePlanOptions {
    ...
    #[test]
    fn largest_share_confirmation_is_blocking_only_when_opted_in() {
        let db = db_with_bundle();
        db.set_ballot_intent(ROUND, 2, Decision::Choice(1), 3)
            .unwrap();
        confirm_vote_fixture(&db, 0, 2, 1);
        ...
        let default_plan = resume_plan(&db, ROUND, &[2]).unwrap();
        assert!(!default_plan.blocking_recovery);

        let waiting_plan =
            resume_plan_with_options(&db, ROUND, &[2], require_largest_share_confirmation())
                .unwrap();

        assert_eq!(
            waiting_plan.next_steps,
            vec![NextStep::ConfirmShare {
                bundle_index: 0,
                proposal_id: 2,
                share_index: 1,
        ...
        assert!(!waiting_plan.blocking_share_work);
        assert!(!waiting_plan.completed_for_display);
        assert_eq!(waiting_plan.primary_action, RoundPlanAction::SubmitShares);
        assert!(waiting_plan.recovered_vote_work.is_empty());

        db.mark_share_confirmed(ROUND, 0, 2, 1).unwrap();
        let confirmed_plan =
            resume_plan_with_options(&db, ROUND, &[2], require_largest_share_confirmation())
                .unwrap();
    ...
    #[test]
    fn largest_share_confirmation_waits_for_all_proposal_decisions() {
        let db = db_with_bundle();
        db.set_ballot_intent(ROUND, 2, Decision::Choice(1), 3)
            .unwrap();
        confirm_vote_fixture(&db, 0, 2, 1);
        ...
        let open_plan =
            resume_plan_with_options(&db, ROUND, &[1, 2], require_largest_share_confirmation())
                .unwrap();

        assert_eq!(open_plan.open_proposals, vec![1]);
        assert!(!open_plan.blocking_recovery);
        assert!(open_plan.needs_draft_setup);
        ...
        assert!(decided_plan.open_proposals.is_empty());
        assert!(decided_plan.blocking_recovery);
        assert_eq!(
            decided_plan.next_steps.first(),
            Some(&NextStep::ConfirmShare {
                bundle_index: 0,
                proposal_id: 2,
                share_index: 1,
            })
    ...
    #[test]
    fn non_largest_share_work_stays_nonblocking_when_opted_in() {
        let db = db_with_bundle();
        db.set_ballot_intent(ROUND, 2, Decision::Choice(1), 3)
            .unwrap();
        confirm_vote_fixture(&db, 0, 2, 1);
    ...
    #[test]
    fn missing_largest_share_stays_blocking_when_opted_in() {
        let db = db_with_bundle();
        db.set_ballot_intent(ROUND, 2, Decision::Choice(1), 3)
            .unwrap();
        confirm_vote_fixture(&db, 0, 2, 1);
