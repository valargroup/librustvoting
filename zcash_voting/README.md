...
- Use `delegate::submission` with `DelegationSigner::signature(sig, sighash)`,
  ...
  signature plus the signed sighash. Integrations that run the crate inside that
  same seed trust boundary can use `PreparedSigner::from_wallet_seed` to derive
  the randomized account SpendAuth key and sign the request.
